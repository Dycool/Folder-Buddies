#!/usr/bin/env bash
# Derive all platform-specific app icons from the canonical repo-root icon.png.
set -euo pipefail
...
if [ ! -f "$SRC" ]; then
  ...
  echo "Put the canonical app icon at: $ROOT_DIR/icon.png" >&2
  exit 1
fi
...
if command -v iconutil >/dev/null 2>&1 && command -v sips >/dev/null 2>&1; then
  ...
  rm -rf "$OUT_DIR/icon.iconset"
  echo "wrote branding/icon.icns (iconutil)"
...
PY
then
  echo "wrote branding/icon.icns (Pillow)"
else
  echo "note: could not write branding/icon.icns (need iconutil or python3 with Pillow)"
fi
