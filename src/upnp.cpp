...
bool Upnp::map(uint16_t internalPort, std::string& externalIp, uint16_t& externalPort,
    ...
    // Scan for a free external port. A port already mapped on the router (by
    // another instance or unrelated software) is skipped, and AddPortMapping
    // failing for a port is treated the same way — both mean "try the next".
    // The rule name carries the external port so two instances never collide.
    bool mapped = false;
