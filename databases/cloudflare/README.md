...
The workflow generates `databases/cloudflare/wrangler.ci.toml` at runtime from those secrets. That file is ignored and must never be committed.
...
Validation runs on pull requests and pushes touching `databases/cloudflare/**`:

- public-safe `wrangler.toml` guard
- `node --check databases/cloudflare/worker.mjs`
- C++23 build of `databases/cloudflare/worker_core.cpp`
