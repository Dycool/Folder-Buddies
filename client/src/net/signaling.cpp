...
constexpr qint64 kFirebaseRoomMaxAgeMs = 30LL * 24 * 60 * 60 * 1000;

qint64 firebase_created_at_ms(const QJsonObject& o) {
    const QJsonValue v = o.value("createdAt");
...
bool firebase_room_expired(const QJsonObject& o) {
    const qint64 createdAtMs = firebase_created_at_ms(o);
    return createdAtMs <= 0 ||
           createdAtMs < QDateTime::currentMSecsSinceEpoch() - kFirebaseRoomMaxAgeMs;
}
...
bool FirebaseSignalingClient::get(const std::string& lookupId, std::string& salt, std::string& wrapped,
    ...
    if (firebase_room_expired(o)) {
        std::string ignored;
        remove(lookupId, "", ignored);
        err = "Firebase fallback room expired";
        return false;
    }
